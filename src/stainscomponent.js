...
export default class extends React.Component {
  componentDidMount() {  
    ...
    if (gl) {
      ...
      this._stainsAnimation.setBrush('stain');
...
/*
    ...
    let notes = [];
    let latestPedalValue = null;

    midihandler.onNote((note) => {
        notes.push(note);
    })

    midihandler.onHoldPedal((controller) => {
      let pedalValue = controller.value / 127;
      this._stainsAnimation.setSpeeds({
        dryingSpeed: (1 - pedalValue) * 0.05 + 0.001,
        evaporationSpeed: (1 - pedalValue) * 0.05 + 0.002
      });
    });

    let sizeCoefficient = 2.0;

    midihandler.onExpressionPedal((controller) => {
      let pedalValue = controller.value / 127;
      sizeCoefficient = 0.3 + pedalValue * 8.0;
    });
    ...
    this._stainsAnimation.doEachFrame(() => {
      ...
      notes.forEach((note) => {
        ...
        const number = note.note.number;
        

        const position = [((note.note.number%(21))) / (21) + 2/12 * Math.random(),
          ((note.note.number%(17))) / (17) + 1/12 * Math.random()];
        const size = note.velocity * 5 * sizeCoefficient;
        const amount = 0.5 + Math.random() * 0.5 * 5;
        const concentration = Math.random() * 0.2;
        const fuzziness = note.velocity;

        console.log(note.velocity);



        this._stainsAnimation.setColor([
          (note.note.number % 12) / 12,
          ((note.note.number + 5) % 14) / 24,
          ((note.note.number + 8) % 16) / 12]);

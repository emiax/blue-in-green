...
import SpeedBrush from 'stains/src/brushes/speedbrush';
...
class StainAnimation {
  constructor(gl, w, h, imageSource, backgroundColor) {
    ...
    this._speedBrush = new SpeedBrush({
      context: this._context,
      simulator: this._simulator
    });
    ...
    if (!this._simulator.init()) {
      ...
      console.error('could not setup simulator');
      return;
    }
  ...
  step() {
    ...
    if (this._status === 'on') {
      ...
      this._stainQueue = [];
      this._speedQueue.forEach((speedSpecfication) => {
        this._speedBrush.apply(speedSpecfication);
      });
      this._speedQueue = [];
